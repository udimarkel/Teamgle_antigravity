      ...
      <div class="row justify-between items-center" v-if="title">
        ...
        <div class="row q-gutter-x-sm" v-if="showActions">
          <q-btn
            outline
            color="grey-7"
            :label="langStore.t('Boost Requests')"
            no-caps
          />
          <q-btn
            outline
            color="grey-7"
            :label="langStore.t('Publish Staffing Request')"
            no-caps
          />
        </div>
      </div>
        ...
        >
            ...
            >
              <q-card bordered flat class="q-mb-sm">
                ...
                >
                  <div class="row items-center">
                    ...
                    <div>
                      ...
                      <div class="text-caption text-grey-7">
                        {{ props.row.role }} • {{ props.row.shiftCategory }}
                      </div>
                    </div>
                  </div>
                  ...
                  <div
                    v-if="
                      props.row.status === 'Applicant' && props.row.appliedAt
                    "
                    class="text-caption text-weight-bold"
                    :class="getTimeColor(props.row.appliedAt)"
                  >
                    {{ formatTimeSince(props.row.appliedAt) }}
                  </div>
                  <div
                    v-else-if="
                      ['Standby', 'Approved'].includes(props.row.status) &&
                      props.row.statusChangedAt
                    "
                    class="text-caption text-weight-bold"
                    :class="getTimeColor(props.row.statusChangedAt)"
                  >
                  ...
                  <div class="row items-center justify-between">
                    <div class="row items-center q-gutter-x-sm">
                      <q-badge
                        ...
                        :text-color="
                          getFrequencyTextColor(props.row.workFrequency)
                        "
                      >
                      ...
                      <!-- Warnings -->
                      <q-icon
                        v-if="props.row.hasWarning"
                        name="error_outline"
                        color="red"
                        size="xs"
                      />
                      <q-icon
                        v-if="props.row.hasDoc"
                        name="description"
                        color="blue"
                        size="xs"
                      />
                ...
                <q-card-actions align="right" class="q-pa-sm bg-grey-1">
                  <!-- 1. APPLICANT -->
                  <div
                    v-if="props.row.status === 'Applicant'"
                    class="row q-gutter-x-sm full-width justify-end"
                  >
                    <q-btn
                      flat
                      class="bg-white"
                    ...
                    <q-btn
                      ...
                      color="purple"
                      size="sm"
                      @click="$emit('hold', props.row)"
                    />
                  </div>

                  <!-- 2. STANDBY -->
                  <div v-else-if="props.row.status === 'Standby'">
                    <span class="text-orange-8 text-weight-bold q-mr-sm"
                      >Standby</span
                    >
                  </div>
                  ...
                  <div v-else-if="props.row.status === 'Approved'">
                    <span class="text-green-8 text-weight-bold q-mr-sm"
                      >Approved</span
                    >
                  </div>
          ...
          <template v-slot:body-cell-actions="props">
            <q-td :props="props" class="text-right">
              ...
              <div
                v-else-if="props.row.status === 'Standby'"
                class="row items-center justify-end no-wrap"
              >
          ...
          <template v-slot:body-cell-time_elapsed="props">
            <q-td :props="props" class="text-center">
              ...
              <div
                v-if="props.row.status === 'Applicant' && props.row.appliedAt"
...
const columns = computed(() => [
  ...
  {
    name: "role",
    label: langStore.t("APPLIED ROLE"),
    align: "left",
    field: "role",
    sortable: true,
  },
  {
    name: "shiftTime",
    label: langStore.t("APPLIED SHIFT"),
    align: "left",
    field: "shiftTime",
  },
  {
    name: "cost",
    label: langStore.t("Cost (H)"),
    align: "center",
    field: "hourlyRate",
  },
  {
