                  ...
                  >
                    <q-list style="min-width: 250px">
                      <q-item-label header class="text-weight-bold"
                        >Events Running Today</q-item-label
                      >
                      ...
                      <q-item
                        ...
                        clickable
                        v-close-popup
                        @click="startGoLive(evt.id)"
                      >
                        <q-item-section avatar>
                          <q-avatar
                            size="28px"
                            :color="
                              evt.statusColor.split(' ')[0].replace('bg-', '')
                            "
                            text-color="white"
                            icon="event"
                          />
                      ...
                      <q-item v-if="todaysEvents.length === 0">
                        <q-item-section
                          class="text-center text-grey text-italic q-pa-sm"
                          >No events found for today</q-item-section
                        >
                      </q-item>
                    </q-list>
                  </q-menu>
        ...
        <div class="row q-mt-md">
          <div class="col-12">
            <q-card class="dashboard-card">
              <q-card-section
                class="todo-header row items-center justify-between"
              >
              ...
              <q-list separator>
                <q-item
                  ...
                  v-ripple
                  tag="label"
                  @dblclick="navigateToEventTasks(task)"
                >
                  ...
                  <q-item-section v-if="!task.isEditing">
                    <q-item-label
                      :class="{ 'text-strike text-grey-5': task.isCompleted }"
        ...
        <q-card class="dashboard-card">
          <q-card-section class="row justify-between items-center">
            <div class="card-title">
              {{ langStore.t("Upcoming Activities") }}
            </div>
            <router-link to="/events" class="view-more-link">{{
              langStore.t("View more")
            }}</router-link>
          </q-card-section>
          ...
          <q-list padding class="scrollable-list">
            ...
            >
              <q-item-section>
                <q-item-label>
                  <q-badge
                    :class="getTimeBadge(event.date).color"
                    rounded
                    class="q-mb-xs"
                    >{{ getTimeBadge(event.date).label }}</q-badge
                  >
                </q-item-label>
                ...
                <q-item-label caption class="q-mt-xs">
                  <q-icon name="event" /> {{ event.date }} &bull;
                  <q-icon name="schedule" /> {{ event.timeRange }}
                </q-item-label>
                <div class="row q-mt-sm">
                  <q-avatar
                    v-for="(avatar, i) in event.workersAssigned"
                    :key="i"
                    size="24px"
                    class="worker-avatar"
                  >
...
import {
  getFinancialData,
  FinancialData,
} from "../services/mock/financialController";
...
import {
  ...
  addTask,
  updateTask,
} from "../services/mock/todoController";
